...
function StatusBadge({ f }: { f: UsnFile }) {
  if (f.status === 'ready') {
    return (
...
function DropZone({ caseId }: { caseId: string }) {
  ...
  const upload = useMutation({
    mutationFn: (file: File) => usnApi.upload(caseId, file),
    onSuccess:  () => {
      qc.invalidateQueries({ queryKey: ['usn-files', caseId] })
      setErr(null)
    },
      ...
      >
        ...
        {upload.isPending ? (
          ...
          <div className="flex flex-col items-center gap-1.5">
            ...
            <p className="text-[11px] text-accent-muted">Drop <span className="font-mono text-white/60">MFTECmd $J CSV</span> here</p>
...
export default function UsnFileList({ caseId, selectedFileId, onSelectFile }: Props) {
  const qc = useQueryClient()

  const { data: files = [] } = useQuery({
    queryKey:       ['usn-files', caseId],
    queryFn:        () => usnApi.listFiles(caseId),
    refetchInterval: (query) => {
      const data = query.state.data as UsnFile[] | undefined
      return data?.some(f => f.status === 'pending' || f.status === 'parsing') ? 1500 : false
    },
  })

  const remove = useMutation({
    mutationFn: (id: string) => usnApi.deleteFile(caseId, id),
    onSuccess:  () => qc.invalidateQueries({ queryKey: ['usn-files', caseId] }),
  })

  const reparse = useMutation({
    mutationFn: (id: string) => usnApi.reparse(caseId, id),
    onSuccess:  () => qc.invalidateQueries({ queryKey: ['usn-files', caseId] }),
  })

  const addEvidence = useMutation({
    mutationFn: (id: string) => usnApi.addEvidence(caseId, id),
    onSuccess:  () => qc.invalidateQueries({ queryKey: ['usn-files', caseId] }),
  })
