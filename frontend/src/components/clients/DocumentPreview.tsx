...
function SpreadsheetTable({ rows }: { rows: string[][] }) {
  if (rows.length === 0) {
    return <p className="text-ui text-fg-secondary p-6">Empty file.</p>
  }
...
function XlsxPreview({ blob }: { blob: Blob }) {
  ...
  if (error) return <p className="text-ui text-severity-critical p-6">{error}</p>
  if (!workbook) return <p className="text-ui text-fg-secondary p-6">Loading...</p>

  return (
    <div className="flex flex-col h-full">
      {workbook.SheetNames.length > 1 && (
        <div className="flex items-center gap-1 px-3 py-1.5 border-b border-hairline shrink-0 overflow-x-auto">
          {workbook.SheetNames.map(name => (
            <button
              key={name}
              onClick={() => setSheet(name)}
              className={`text-label px-2.5 py-1 rounded-control whitespace-nowrap transition-colors ${ sheet === name ? 'bg-accent/10 text-accent' : 'text-fg-secondary/60 hover:text-fg'
              }`}
            >
              {name}
            </button>
...
function CsvPreview({ blob }: { blob: Blob }) {
  ...
  if (error) return <p className="text-ui text-severity-critical p-6">{error}</p>
  if (!rows) return <p className="text-ui text-fg-secondary p-6">Loading...</p>
...
export default function DocumentPreview({ clientId, doc, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={onClose}>
      <div
        className="w-[90vw] h-[85vh] max-w-6xl bg-panel border border-hairline shadow-2xl flex flex-col overflow-hidden"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center gap-3 px-4 py-3 border-b border-hairline shrink-0">
          <div className="flex-1 min-w-0">
            <p className="text-ui font-semibold text-fg truncate">{doc.name}</p>
            <p className="text-label text-fg-secondary/50 font-mono">{doc.file_name} · {fmtBytes(doc.file_size)}</p>
          </div>
          <button
            onClick={handleDownload}
            disabled={!blob}
            className="flex items-center gap-1.5 text-label text-fg-secondary/60 hover:text-fg transition-colors disabled:opacity-30"
          >
            <Download size={13} /> Download
          </button>
          <button onClick={onClose} className="text-fg-secondary hover:text-fg transition-colors p-1">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="flex-1 min-h-0 bg-canvas">
          {error ? (
            <div className="flex flex-col items-center justify-center h-full gap-2 text-severity-critical">
              <AlertCircle size={24} />
              <p className="text-ui">{error}</p>
            </div>
          ) : !blob ? (
            <div className="flex items-center justify-center h-full text-fg-secondary text-ui">Loading...</div>
          ) : isPdf && blobUrl ? (
            ...
            <div className="flex flex-col items-center justify-center h-full gap-3 text-fg-secondary">
              <FileQuestion size={32} className="text-fg-secondary/30" />
              <p className="text-ui">Preview not available for this file type.</p>
              <button onClick={handleDownload} className="btn-secondary text-label flex items-center gap-1.5">
