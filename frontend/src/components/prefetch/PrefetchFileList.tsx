...
import { prefetchApi, type PrefetchFile } from '../../api/prefetch'
...
interface Props {
  ...
  onSelectFile:   (file: PrefetchFile) => void
}

// ── Status badge ──────────────────────────────────────────────────────────────

function StatusBadge({ f }: { f: PrefetchFile }) {
  if (f.status === 'ready') {
    return (
...
function DropZone({ caseId }: { caseId: string }) {
  ...
  const onDrop = useCallback(async (accepted: File[]) => {
    ...
    setUploading(true)
    try {
      await Promise.all(accepted.map(f => prefetchApi.upload(caseId, f)))
      qc.invalidateQueries({ queryKey: ['prefetch-files', caseId] })
    } catch {
        ...
        {uploading ? (
          ...
          <div className="flex flex-col items-center gap-1.5">
            ...
            <p className="text-[11px] text-accent-muted">
              Drop <span className="font-mono text-white/60">PECmd CSV</span> here
            </p>
            <p className="text-[10px] text-accent-muted/40">
              Run PECmd.exe -d Prefetch --csv out --csvf prefetch.csv
            </p>
          </div>
        )}
      </div>
...
export default function PrefetchFileList({ caseId, selectedFileId, onSelectFile }: Props) {
  const qc = useQueryClient()

  const { data: files = [] } = useQuery({
    queryKey:        ['prefetch-files', caseId],
    queryFn:         () => prefetchApi.listFiles(caseId),
    refetchInterval: (query) => {
      const data = query.state.data as PrefetchFile[] | undefined
      return data?.some(f => f.status === 'pending' || f.status === 'parsing') ? 1500 : false
    },
  })

  const remove = useMutation({
    mutationFn: (id: string) => prefetchApi.deleteFile(caseId, id),
    onSuccess:  () => qc.invalidateQueries({ queryKey: ['prefetch-files', caseId] }),
  })

  const reparse = useMutation({
    mutationFn: (id: string) => prefetchApi.reparse(caseId, id),
    onSuccess:  () => qc.invalidateQueries({ queryKey: ['prefetch-files', caseId] }),
  })

  const addEvidence = useMutation({
    mutationFn: (id: string) => prefetchApi.addEvidence(caseId, id),
    onSuccess:  () => qc.invalidateQueries({ queryKey: ['prefetch-files', caseId] }),
  })
