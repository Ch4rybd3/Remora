  ...
  return (
    <div className="flex items-center gap-1 px-2 py-0.5">
      <input
        ...
        className="flex-1 bg-fg/5 border border-hairline rounded-control px-1.5 py-0.5 text-label text-fg outline-none focus:border-accent/40"
      />
      <button onClick={() => val.trim() && onConfirm(val.trim())} className="text-accent hover:text-accent/70">
        <Check size={11} />
      </button>
      <button onClick={onCancel} className="text-fg-secondary/60 hover:text-fg">
        <X size={11} />
      </button>
    </div>
  )
...
function TreeNode({
    ...
    return (
      <div>
        <div
          ...
          className="group flex items-center gap-1.5 py-0.5 pr-2 rounded-control cursor-pointer hover:bg-fg/5 transition-colors"
          style={{ paddingLeft: `${indent + 8}px` }}
          onClick={() => setOpen(o => !o)}
        >
          <span className="shrink-0 text-fg-secondary/60">
            {open ? <ChevronDown size={12} /> : <ChevronRight size={12} />}
          </span>
          <span className="shrink-0 text-fg-secondary/70">
            {open ? <FolderOpen size={13} /> : <Folder size={13} />}
          </span>
          <span className="flex-1 text-label text-fg/70 truncate">{node.name}</span>
          {hovered && (
            <span className="flex items-center gap-0.5 shrink-0" onClick={e => e.stopPropagation()}>
              <button
                onClick={() => { setOpen(true); setCreating('file') }}
                className="p-0.5 rounded-control text-fg-secondary/40 hover:text-accent transition-colors"
                title="New note"
              ><Plus size={10} /></button>
              <button
              ...
              <button
                ...
                className="p-0.5 rounded-control text-fg-secondary/40 hover:text-severity-critical transition-colors"
                title="Delete folder"
  ...
  return (
    <div
      ...
      className={`group flex items-center gap-1.5 py-0.5 pr-2 rounded-control cursor-pointer transition-colors ${ isActive
          ? 'bg-accent/10 text-accent'
          : 'hover:bg-fg/5 text-fg/60 hover:text-fg/80'
      }`}
      style={{ paddingLeft: `${indent + 8}px` }}
    >
      <FileText size={12} className="shrink-0 opacity-60" />
      <span className="flex-1 text-label truncate">{node.name}</span>
      {hovered && (
        <span className="flex items-center gap-0.5 shrink-0" onClick={e => e.stopPropagation()}>
          <button
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-3 py-2.5 border-b border-hairline flex items-center justify-between shrink-0">
        <span className="text-label font-semibold tracking-widest uppercase text-fg-secondary/50">Vault</span>
        <div className="flex items-center gap-1">
          <button
            ...
            className="p-1 rounded-control text-fg-secondary/50 hover:text-fg hover:bg-fg/5 transition-colors"
          ><ChevronsUpDown size={13} /></button>
          <button
            onClick={() => setCreatingRoot('file')}
            title="New note"
            className="p-1 rounded-control text-fg-secondary/50 hover:text-accent hover:bg-accent/5 transition-colors"
          ><Plus size={13} /></button>
          <button
          ...
          <button
            ...
            className="p-1 rounded-control text-fg-secondary/50 hover:text-accent hover:bg-accent/5 transition-colors text-label font-mono"
          >ZIP</button>
      ...
      <div className="flex-1 overflow-y-auto py-1 px-1">
        ...
        {nodes.length === 0 ? (
          <div className="px-3 py-8 text-center">
            <p className="text-label text-fg-secondary/40 italic">Vault is empty.</p>
            <p className="text-label text-fg-secondary/30 mt-1">Import a .zip or create a note.</p>
          </div>
        ) : (
          nodes.map(node => (
            <TreeNode
