...
def update_playbook(pb_id: str, payload: PlaybookUpdate, db: Session = Depends(get_db)):
    pb = db.query(Playbook).filter(Playbook.id == pb_id).first()
    if not pb:
        raise HTTPException(status_code=404, detail="Playbook introuvable")
...
def delete_playbook(pb_id: str, db: Session = Depends(get_db)):
    pb = db.query(Playbook).filter(Playbook.id == pb_id).first()
    if not pb:
        raise HTTPException(status_code=404, detail="Playbook introuvable")
    db.delete(pb)
...
def attach_playbook(case_id: str, payload: CasePlaybookCreate, db: Session = Depends(get_db)):
    _get_case(case_id, db)
...
def detach_playbook(case_id: str, cp_id: str, db: Session = Depends(get_db)):
